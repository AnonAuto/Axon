...
# reset callback functions.  Used in some cases (e.g. lib/network/config.sh)
reset_cb() {
	config_cb() { return 0; }
	option_cb() { return 0; }
	list_cb() { return 0; }
...
# functions to process UCI sections via eval
package() {
	return 0
}
...
# for each section of the current file that is type $2, call function $1
config_foreach() {
	local function="$1"
	[ "$#" -ge 1 ] && shift
	local type="$1"
	...
	for section in ${CONFIG_SECTIONS}; do
	        echo processing configuration $section:
		config_get cfgtype "$section" TYPE
		[ -n "$type" -a "x$cfgtype" != "x$type" ] && continue
		echo eval "$function \"\$section\" \"\$@\""
