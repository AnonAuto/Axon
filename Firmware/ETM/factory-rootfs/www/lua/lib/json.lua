    ...
    if bArray then
      ...
      for i,j in base.pairs(v) do
        if isEncodable(i) and isEncodable(j) then
          table.insert(rval, indentstr .. '"' .. encodeString(i) .. '":' .. encode(j, level + 1 ) )
        end
      end
    end
    if bArray then
      ...
      if pretty == 1 and level > 0 then
         indent = (level-1)*indentsize;
      else
         indent = 0;
      end

      return '{' .. newline .. table.concat(rval,','..newline) .. newline .. string.rep(' ', indent) .. '}'
    end
  end
